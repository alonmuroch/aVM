...
rust-vm/
├── crates/
│   ├── bootloader/    # Guest bootloader and test harness integration
│   ├── kernel/        # Guest kernel (tasks, syscalls, memory layout)
...
```
┌───────────────┐   ┌───────────────────┐   ┌──────────────────┐
│  Bootloader   │─▶│      Kernel       │─▶│   User Tasks     │
│ (BootInfo,    │  │  init, traps,     │  │ (addr space,    │
│  VM setup)    │  │  syscalls)        │  │  trapframe)     │
└───────────────┘  └─────────┬─────────┘   └────────┬─────────┘
                             │                      │
                             v                      v
                   ┌───────────────────┐   ┌──────────────────┐
                   │ State + Storage  │   │   RISC-V VM       │
                   │ (accounts,       │   │ (CPU + decoder)  │
                   │  receipts)       │   └──────────────────┘
                   └───────────────────┘
```
